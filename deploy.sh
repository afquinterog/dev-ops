...
SERVER_KEY="${SERVER_KEY:-aws-ubuntu-server-key.pem}"
SSH_USER="${SSH_USER:-ubuntu}"
KEY_USER="${KEY_USER:-ubuntu}"
SSH_EMAIL="andres@mkitdigital.com"
DOCKER_VERSION="${DOCKER_VERSION:-17.05.0}"

DOCKER_PULL_IMAGES=("postgres" "redis" "elasticsearch:1.7.5" "schickling/mailcatcher")
COPY_UNIT_FILES=("swap" "postgres" "redis" "mailcatcher" "elasticsearch" "nginx" "entoro-api")
...
function create_ssh_key_on_server(){
  echo "Creating a ssh key on the server to connect external systems (github, etc)"
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
      ssh-keygen -t rsa -b 4096 -C "${SSH_EMAIL}" -q -N ""
      eval "$(ssh-agent -s)"
...
function install_docker () {
  ...
  ssh -i ${SERVER_KEY} -t "${SSH_USER}@${SERVER_IP}" bash -c "'
sudo apt-get update
sudo apt-get install -y -q libapparmor1 aufs-tools ca-certificates libltdl7
wget -O "docker.deb https://apt.dockerproject.org/repo/pool/main/d/docker-engine/docker-engine_${1}~ce-0~ubuntu-xenial_amd64.deb"
...
function docker_pull () {
  ...
  do
    ssh -i ${SERVER_KEY} -t "${SSH_USER}@${SERVER_IP}" bash -c "'docker pull ${image}'"
  done
  echo "done!"
}
...
function configure_nginx () {
  ...
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
        sudo rm -rf /docker
        sudo git clone https://github.com/afquinterog/docker-images.git /docker
        sudo chown -R ${SSH_USER}:${SSH_USER} /docker
        sudo chmod -R 775 /docker
        docker build -t nginx:latest /docker/nginx
        docker stop nginx
...
function configure_app () {
  ...
  ssh -t "${SSH_USER}@${SERVER_IP}" bash -c "'
        sudo rm -rf /myapp
        sudo git clone https://github.com/ventureco/api.git /myapp
        cd /myapp
        sudo git checkout mkit-aws-integration
        sudo chown -R ${SSH_USER}:${SSH_USER} /myapp
        sudo chmod -R 775 /myapp
        
        docker build -t entoro-api:latest .
        docker stop entoro-api
        docker rmi $(docker images --quiet --filter "dangling=true")
  '"
}
...
sudo systemctl enable redis.service
sudo systemctl start redis.service
sudo systemctl enable entoro-api.service
sudo systemctl start entoro-api.service
...
case "${1}" in
  ...
  shift
  ;;
  -sk|--ssh-key-on-server)
  create_ssh_key_on_server
  shift
  ;;
  -d|--docker)
